...
class TrainingSummary:
    ...
    class ModelCard:
    def __init__(self, model_name, source, finetuned_from=None, dataset=None, eval_results=None, hyperparameters=None):
    ...
    def create_metadata(self):
        metadata_dict = {}
        if self.finetuned_from:
            metadata_dict["Fine-tuned from"] = self.finetuned_from

        if self.dataset:
            if isinstance(self.dataset, str):
                metadata_dict["Dataset"] = self.dataset
            elif isinstance(self.dataset, (tuple, list)):
                metadata_dict["Datasets"] = self.dataset

        if self.eval_results:
            metadata_dict["Evaluation Results"] = self.eval_results

        if self.hyperparameters:
            metadata_dict["Hyperparameters"] = self.hyperparameters
    ...
    def format_metadata(self, metadata_dict):
        ...
        if metadata_dict:
            metadata_text += "---\n"
            metadata_text += yaml.dump(metadata_dict, sort_keys=False)
            metadata_text += "---\n\n"
        return metadata_text


    def to_model_card(self):
        metadata_dict = self.create_metadata()
        metadata_text = self.format_metadata(metadata_dict)
        ...
        if self.finetuned_from is None:
            ...
            model_card += (
                f"This model is a fine-tuned version of [{self.finetuned_from}]"
                f"(https://huggingface.co/{self.finetuned_from}) on "
            )
        ...
        if self.eval_results:
            ...
            for k, v in self.eval_results.items():
                model_card += f"- {k}: {_maybe_round(v)}\n"

        model_card += f"\n\n## Model Description\n\nMore information needed\n\n"
        model_card += f"## Intended Uses & Limitations\n\nMore information needed\n\n"
        model_card += f"## Training & Evaluation Data\n\nMore information needed\n\n"
        model_card += f"## Training Procedure\n\n"
        model_card += f"### Training Hyperparameters\n\n"
        if self.hyperparameters:
            for hp, value in self.hyperparameters.items():
                model_card += f"- {hp}: {value}\n"
            model_card += "\n"
        else:
            model_card += "More information needed\n\n"

        model_card += f"\n\n## Framework & Hardware Parameters\n\n"
        model_card += f"- Transformers Version: {__version__}\n"

        if self.source == "trainer" and is_torch_available():
            model_card += f"- Pytorch Version: {torch.__version__}\n"
        elif self.source == "keras" and is_tf_available():
            model_card += f"- TensorFlow Version: {tf.__version__}\n"

        if is_datasets_available():
            model_card += f"- Datasets Version: {datasets.__version__}\n"

        if is_tokenizers_available():
            model_card += f"- Tokenizers Version: {tokenizers.__version__}\n"

        return metadata_text + model_card
